...
class StockDataPreparer:
    ...
    def _prepare_china_stock_data(self, stock_code: str, period_days: int,
        ...
        try:
            ...
            stock_info_dict = manager.get_stock_info(stock_code)
            
            # 将字典转换为字符串格式以便后续处理
            stock_info = f"股票名称: {stock_info_dict.get('name', '未知')}\n"
            stock_info += f"股票代码: {stock_info_dict.get('symbol', stock_code)}\n"
            stock_info += f"所属行业: {stock_info_dict.get('industry', '未知')}\n"
            stock_info += f"上市时间: {stock_info_dict.get('list_date', '未知')}"
    ...
    def _prepare_hk_stock_data(self, stock_code: str, period_days: int,
        ...
        try:
            ...
            from tradingagents.dataflows.data_source_manager import get_hk_stock_info_unified
            ...
            from tradingagents.dataflows.data_source_manager import get_hk_stock_data_unified
