...
"""
    ...
    @staticmethod
    @tool
    def get_stock_news_crawler(
        ticker: Annotated[str, "股票代码，如300663, 000001等"],
        max_days: Annotated[int, "获取多少天内的新闻，默认14天"] = 14,
    ) -> str:
        """
        通过网页爬虫获取股票相关新闻，专门针对中国股市优化。
        从新浪财经和东方财富等权威网站爬取最新新闻和公告。
        ...
        Args:
            ticker (str): 股票代码，如300663, 000001
            max_days (int): 获取天数，默认14天
        Returns:
            str: 格式化的新闻数据，包含标题、内容、时间、来源等信息
        """
        try:
            from tradingagents.news_crawler import NewsCrawler
            crawler = NewsCrawler()
            return crawler.get_formatted_news_text(ticker, max_days)
        except Exception as e:
            return f"❌ 新闻爬虫获取失败: {e}"
