...
class FinancialSituationMemory:
    def __init__(self, name, config):
        ...
        if self.llm_provider == "dashscope" or self.llm_provider == "alibaba":
            ...
            # DeepSeek不提供embedding服务，使用优先级降级策略
            # 优先级：SiliconFlow > 阿里百炼 > OpenAI > 禁用
            
            # 1. 首先尝试SiliconFlow embedding
            siliconflow_key = os.getenv('LITELLM_PROXY_API_KEY')
            siliconflow_base = os.getenv('LITELLM_PROXY_API_BASE')
            siliconflow_model = os.getenv('EMBEDDING_MODEL', 'litellm_proxy/BAAI/bge-m3')
            
            if siliconflow_key and siliconflow_base:
                try:
                    self.embedding = siliconflow_model
                    self.client = OpenAI(
                        api_key=siliconflow_key,
                        base_url=siliconflow_base
                    )
                    logger.info(f"✅ DeepSeek使用SiliconFlow嵌入服务 (模型: {siliconflow_model})")
                except Exception as e:
                    logger.error(f"⚠️ SiliconFlow嵌入初始化失败: {e}")
                    siliconflow_key = None  # 强制降级
            
            # 2. 如果SiliconFlow不可用，检查是否强制使用OpenAI嵌入
            if not siliconflow_key:
                ...
                if not dashscope_key or force_openai:
                    ...
                    if openai_key:
                        ...
                        # 4. 最后选择：禁用内存功能
                        self.client = "DISABLED"
                        logger.warning(f"🚨 未找到可用的嵌入服务，内存功能已禁用")
                        logger.info(f"💡 系统将继续运行但不保存历史记忆")
    ...
    def get_embedding(self, text):
        ...
        if (self.llm_provider == "dashscope" or
            ...
            try:
                ...
                # 根据embedding模型类型显示不同的成功消息
                if "litellm_proxy" in self.embedding:
                    logger.debug(f"✅ SiliconFlow embedding成功，模型: {self.embedding}，维度: {len(embedding)}")
                else:
                ...
                # API调用方法不存在
                service_name = "SiliconFlow" if "litellm_proxy" in self.embedding else "OpenAI"
                logger.error(f"❌ {service_name} API调用错误: {str(e)}")
                logger.warning(f"⚠️ 记忆功能降级，返回空向量")
                return [0.0] * 1024
                ...
                # 网络连接错误
                service_name = "SiliconFlow" if "litellm_proxy" in self.embedding else "OpenAI"
                logger.error(f"❌ {service_name}网络连接错误: {str(e)}")
                logger.warning(f"⚠️ 记忆功能降级，返回空向量")
                return [0.0] * 1024
                ...
                # 请求超时
                service_name = "SiliconFlow" if "litellm_proxy" in self.embedding else "OpenAI"
                logger.error(f"❌ {service_name}请求超时: {str(e)}")
                logger.warning(f"⚠️ 记忆功能降级，返回空向量")
                return [0.0] * 1024
                ...
                # 响应格式错误
                service_name = "SiliconFlow" if "litellm_proxy" in self.embedding else "OpenAI"
                logger.error(f"❌ {service_name}响应格式错误: {str(e)}")
                logger.warning(f"⚠️ 记忆功能降级，返回空向量")
                return [0.0] * 1024
                ...
                # 其他所有异常
                service_name = "SiliconFlow" if "litellm_proxy" in self.embedding else "OpenAI"
                logger.error(f"❌ {service_name} embedding未知异常: {str(e)}")
                logger.warning(f"⚠️ 记忆功能降级，返回空向量")
                return [0.0] * 1024
