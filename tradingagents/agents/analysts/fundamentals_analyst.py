...
def _get_company_name_for_fundamentals(ticker: str, market_info: dict) -> str:
    ...
    try:
        if market_info['is_china']:
            ...
            # 解析股票名称（stock_info是字典格式）
            if isinstance(stock_info, dict) and stock_info.get('name'):
                company_name = stock_info['name']
                logger.debug(f"📊 [基本面分析师] 从统一接口获取中国股票名称: {ticker} -> {company_name}")
                return company_name
            elif isinstance(stock_info, str) and "股票名称:" in stock_info:
                # 兼容字符串格式
                company_name = stock_info.split("股票名称:")[1].split("\n")[0].strip()
                logger.debug(f"📊 [基本面分析师] 从统一接口获取中国股票名称: {ticker} -> {company_name}")
                return company_name
            else:
                logger.warning(f"⚠️ [基本面分析师] 无法从统一接口解析股票名称: {ticker}, 返回数据: {stock_info}")
...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        # 调试日志：记录发送给LLM的内容
        logger.info("🔍 [基本面分析师] 准备调用LLM")
        log_llm_messages("基本面分析师", state["messages"])

        result = chain.invoke(state["messages"])
        
        # 调试日志：记录LLM返回的内容
        logger.info("🔍 [基本面分析师] LLM调用完成")
        log_llm_response("基本面分析师", result)
...
- 分析要详细且专业"""
            ...
            try:
                ...
                # 调试日志：记录第二次LLM调用
                logger.info("🔍 [基本面分析师] 准备进行第二次LLM调用（深度分析）")
                if debug_config.is_llm_debug_enabled():
                    logger.info(f"🔍 [基本面分析师] 分析请求内容长度: {len(analysis_prompt)}")
                    logger.info(f"🔍 [基本面分析师] 分析请求预览: {analysis_prompt[:300]}...")
                
                analysis_result = analysis_chain.invoke({"analysis_request": analysis_prompt})
                
                # 调试日志：记录第二次LLM返回
                logger.info("🔍 [基本面分析师] 第二次LLM调用完成")
                log_llm_response("基本面分析师(深度分析)", analysis_result)
