    ...
    try:
        if market_info['is_china']:
            ...
            if isinstance(stock_info, dict) and stock_info.get('name'):
                ...
                logger.warning(f"⚠️ [DEBUG] 无法从统一接口解析股票名称: {ticker}, 返回数据: {stock_info}")
...
- 投资建议"""
                ...
                # 调试日志：记录发送给LLM的内容
                logger.info("🔍 [市场分析师] 准备调用LLM")
                log_llm_messages("市场分析师", messages)
                ...
                log_llm_response("市场分析师", final_result)
