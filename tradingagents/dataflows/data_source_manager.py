...
class ChinaDataSource(Enum):
    ...
    SOCIAL_MEDIA = "social_media"  # 社交媒体数据源
...
class DataSourceManager:
    ...
    def _check_available_sources(self) -> List[ChinaDataSource]:
        ...
        try:
            ...
            available.append(ChinaDataSource.SOCIAL_MEDIA)
            logger.info("✅ 社交媒体数据源可用")
        except Exception as e:
            logger.warning(f"⚠️ 社交媒体数据源暂时不可用: {e}")
    ...
    def get_social_media_data(self, symbol: str, days: int = 3) -> Dict:
        """
        获取社交媒体情绪数据
        ...
        try:
            # 导入社交媒体接口
            from .interface import get_real_china_social_sentiment
            
            logger.info(f"📱 [社交媒体] 开始获取{symbol}的社交媒体情绪数据...")
            ...
            try:
                result = loop.run_until_complete(get_real_china_social_sentiment(symbol, days))
                logger.info(f"✅ [社交媒体] 成功获取{symbol}的社交媒体情绪数据")
                return result
            finally:
                loop.close()
                
        except Exception as e:
            logger.error(f"❌ [社交媒体] 获取{symbol}社交媒体数据失败: {e}")
            # 返回备用数据
            from .interface import get_fallback_social_sentiment
            import asyncio
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
                result = loop.run_until_complete(get_fallback_social_sentiment(symbol, days))
                return result
            finally:
                loop.close()

    def _run_async_sync(self, async_func, *args, **kwargs):
        """将异步函数包装为同步调用"""
        import asyncio
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
    ...
    def get_stock_info(self, symbol: str) -> Dict:
        ...
        try:
            if self.current_source == ChinaDataSource.TUSHARE:
                ...
                info_str = self._run_async_sync(get_china_stock_info_tushare, symbol)
    ...
    def _parse_stock_info_string(self, info_str, symbol: str) -> Dict:
        """解析股票信息字符串或字典为统一格式"""
        try:
