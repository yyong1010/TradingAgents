...
def check_deepseek_config():
    ...
    if not api_key:
        ...
        print("💡 影响: 主要LLM功能将无法使用")
        return False
    
    print(f"✅ DEEPSEEK_API_KEY已配置: {api_key[:12]}...{api_key[-4:]}")
    ...
    try:
        ...
        if response.choices:
            ...
            print("❌ DeepSeek API测试失败: 无响应")
            return False
            
    except ImportError:
        print("⚠️ openai包未安装，无法测试API")
        return False
...
def check_dashscope_config():
    ...
    if not api_key:
        print("⚠️ DASHSCOPE_API_KEY未配置（可选）")
        print("💡 影响: 无法使用阿里百炼模型，但系统可以正常运行")
        return False
...
def check_memory_functionality():
    ...
    try:
        ...
        # 优先使用DeepSeek，如果没有则尝试DashScope
        if os.getenv('DEEPSEEK_API_KEY'):
            config["llm_provider"] = "deepseek"
        elif os.getenv('DASHSCOPE_API_KEY'):
            config["llm_provider"] = "dashscope"
        else:
            print("⚠️ 记忆功能已禁用（无可用的embedding服务）")
            print("💡 原因: 需要配置DEEPSEEK_API_KEY或DASHSCOPE_API_KEY")
            return False
...
def provide_recommendations(deepseek_ok, dashscope_ok, configured_apis, missing_apis):
    """提供配置建议"""
    print("\n💡 配置建议:")
    print("=" * 50)
    ...
    if not deepseek_ok:
        ...
        print("   - 主要LLM功能将无法使用")
        print("   - 系统无法进行股票分析")
        print("   - 建议配置DEEPSEEK_API_KEY以获得基本功能")
        print()
    
    if not dashscope_ok:
        print("🟡 DashScope未配置（可选）:")
        print("   - 无法使用阿里百炼模型")
        print("   - 记忆功能可能受限")
        print("   - 系统仍可使用DeepSeek正常运行")
        print()
    ...
    if not deepseek_ok and not dashscope_ok:
        print("🔴 严重警告:")
        print("   - 没有配置任何LLM API密钥")
        print("   - 系统无法正常工作")
        print("   - 请至少配置DEEPSEEK_API_KEY")
        print()
    
    print("📋 最小配置建议:")
    print("   1. 配置DEEPSEEK_API_KEY（必需，主要LLM）")
    print("   2. 配置TUSHARE_TOKEN以获得A股数据")
    print("   3. 其他API密钥可选配置")
    print()
    
    print("🚀 完整配置建议:")
    print("   - DEEPSEEK_API_KEY: DeepSeek V3（推荐，性价比高）")
    print("   - TUSHARE_TOKEN: A股专业数据")
    print("   - DASHSCOPE_API_KEY: 阿里百炼（可选，记忆功能）")
