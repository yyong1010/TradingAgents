...
def check_api_keys():
    ...
    details = {
        "DEEPSEEK_API_KEY": {
            ...
            "required": False,  # 不是必需的，因为有其他AI模型选择
    ...
    # 检查是否至少有一个AI模型API密钥
    ai_model_keys = ["DEEPSEEK_API_KEY", "DASHSCOPE_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "GOOGLE_API_KEY"]
    has_ai_model = any(details[key]["configured"] for key in ai_model_keys if key in details)
    
    if not has_ai_model:
        missing_required.append("至少一个AI模型API密钥")
    
    return {
        "all_configured": len(missing_required) == 0 and has_ai_model,
        "required_configured": len(missing_required) == 0 and has_ai_model,
...
def validate_api_key_format(key_type, api_key):
    ...
    # 特定格式检查
    if key_type == "DEEPSEEK_API_KEY":
        if not api_key.startswith("sk-"):
            return False, "DeepSeek API密钥应以'sk-'开头"
    elif key_type == "DASHSCOPE_API_KEY":
        if not api_key.startswith("sk-"):
