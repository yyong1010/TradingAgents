...
# 模块级缓存，避免频繁查询
_latest_analysis_cache = {
    'id': None,
    'timestamp': 0,
    'cache_duration': 5  # 5秒缓存，平衡性能和实时性
}
...
def get_latest_analysis_id() -> Optional[str]:
    ...
    try:
        # 检查缓存是否有效
        current_time = time.time()
        if (current_time - _latest_analysis_cache['timestamp'] < _latest_analysis_cache['cache_duration'] 
            and _latest_analysis_cache['id']):
            logger.debug(f"📊 [缓存命中] 返回缓存的分析ID: {_latest_analysis_cache['id']}")
            return _latest_analysis_cache['id']
        # 检查REDIS_ENABLED环境变量
        redis_enabled = os.getenv('REDIS_ENABLED', 'false').lower() == 'true'
        ...
        if redis_enabled:
            try:
                ...
                if latest_id:
                    # 更新缓存
                    _latest_analysis_cache['id'] = latest_id
                    _latest_analysis_cache['timestamp'] = current_time
                    
                    # 只在首次发现或ID变化时输出INFO日志
                    if _latest_analysis_cache.get('last_logged_id') != latest_id:
                        logger.info(f"📊 [恢复分析] 找到最新分析ID: {latest_id}")
                        _latest_analysis_cache['last_logged_id'] = latest_id
                    else:
        ...
        if data_dir.exists():
            ...
            if progress_files:
                ...
                if filename.startswith("progress_") and filename.endswith(".json"):
                    ...
                    # 更新缓存
                    _latest_analysis_cache['id'] = analysis_id
                    _latest_analysis_cache['timestamp'] = current_time
                    
                    # 只在首次发现或ID变化时输出INFO日志
                    if _latest_analysis_cache.get('last_logged_id') != analysis_id:
                        logger.info(f"📊 [恢复分析] 从文件找到最新分析ID: {analysis_id}")
                        _latest_analysis_cache['last_logged_id'] = analysis_id
                    else:
        ...
        # 没有找到任何分析ID，清空缓存
        _latest_analysis_cache['id'] = None
        _latest_analysis_cache['timestamp'] = 0
        return None
