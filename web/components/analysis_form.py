...
def render_analysis_form():
    ...
    with st.form("analysis_form", clear_on_submit=False):
        ...
        with col2:
            # 研究深度（使用缓存的值）- 默认5级全面分析
            cached_depth = cached_config.get('research_depth', 5) if cached_config else 5
        ...
        # 获取缓存的分析师选择 - 默认全选所有分析师
        cached_analysts = cached_config.get('selected_analysts', ['market', 'social', 'news', 'fundamentals']) if cached_config else ['market', 'social', 'news', 'fundamentals']
