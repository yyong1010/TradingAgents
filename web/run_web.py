...
def check_api_keys():
    ...
    # 检查是否至少有一个AI模型API密钥
    has_ai_model = any([deepseek_key, dashscope_key, openai_key, google_key])
    
    if not has_ai_model or not finnhub_key:
        logger.warning(f"⚠️ API密钥配置不完整")
        logger.info(f"请确保在.env文件中配置以下密钥:")
        if not has_ai_model:
            logger.info(f"  - 至少一个AI模型API密钥:")
            logger.info(f"    * DEEPSEEK_API_KEY (DeepSeek V3，推荐)")
